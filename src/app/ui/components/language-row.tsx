import Glassdiv from "./glass-div";
import Image from "next/image";
import { karla } from "../fonts";

export default function LanguageRow({
	icons,
	names,
}: {
	icons: string[];
	names: string[];
}) {
	return (
		<div className="flex w-full flex-col items-center">
			<Glassdiv className="flex w-full flex-row justify-between rounded-full px-15 py-5 lg:w-3/7">
				{icons.map((src, i) => (
					<div
						key={i}
						className="relative flex h-24 w-24 items-center justify-center"
					>
						<div className="absolute inset-0 rounded-full bg-white/100 shadow-[0_0_2px_2px_rgba(255,255,255,1)] blur-[5px]"></div>
						<Image src={src} alt="icon" fill className="object-contain p-2" />
					</div>
				))}
			</Glassdiv>
			<div className="flex w-full flex-row justify-between px-15 py-2 lg:w-3/7">
				{names.map((name, i) => (
					<div
						key={i}
						className="relative flex w-24 items-center justify-center"
					>
						<h3 className="font-extrabold">{name}</h3>
					</div>
				))}
			</div>
		</div>
	);
}
